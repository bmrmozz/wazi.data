...
namespace wazi.data.clients.mongo {
    public class MongoClient : IObjectClient {
        ...
        public ObjectClientSettings Settings {
            set {
                settings = (MongoClientSettings)value;
            }
        }
        ...
        public void Save<T>(IObjectRepository repo, IEnumerable<T> items, bool autocreate = false) {
            ...
            if (autocreate && !this.HasCollection<T>(repo.FullName))
                this.CreateCollection<T>(repo.FullName);
        ...
        public bool HasCollection<T>(string name) {
            this.Connect();
        ...
        public void CreateCollection<T>(string name) {
            if (this.HasCollection<T>(name))
                return;
            var database = this.client.GetDatabase(this.defaultInstance);
